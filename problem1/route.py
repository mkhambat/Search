...
def successors_cities_bfs_dfs(city, routing_algo, parameter, road_segments):
    ...
    for neighbour_city in road_segments[city[0]]['connected']:
        result.append([neighbour_city, city[0], road_segments[city[0]][neighbour_city][0],
                       road_segments[city[0]][neighbour_city][1]])
    return result
...
def successors_cities_uniform(city, routing_algo, parameter, road_segments):  # succ function for uniform cost
    ...
    for neighbour_city in road_segments[city[1]]['connected']:
        if parameter == "segments":
            ...
            result.append([(float(city[0]) + float(road_segments[city[1]][neighbour_city][0]) / float(
                road_segments[city[1]][neighbour_city][1])), neighbour_city, city[1],
                           ])
            ...
            result.append(
                [(int(city[0]) + int(road_segments[city[1]][neighbour_city][0])) * -1, neighbour_city, city[1]])
    return result
        ...
        if parameter == "segments":
            ...
            if neighbour_city_gps == [0, 0]:  # No coordinates found for neighbour so search for its parent coordinates
                ...
                while parent_gps == [0, 0]:  # continue till you get lat longitude coordinates
                    ...
                    temp = x
                distance_goal = distance_lat_lon(parent_gps[0], parent_gps[1], goal_lat_lon[0],
                                                 goal_lat_lon[1]) - distance
            else:  # distance_goal is distance between goal node and neighbour city
                distance_goal = distance_lat_lon(neighbour_city_gps[0], neighbour_city_gps[1], goal_lat_lon[0],
                                                 goal_lat_lon[1])

        result.append(
            [float(city[0]) + float(road_segments[city[1]][neighbour_city][0]) + distance_goal, neighbour_city,
             city[1]])
    return result
...
if solution != False:
    ...
    for i in result:
        if i == end_city:
            str = str + i
            break
        print("Go from {0} to {1} via {2} for {3} miles".format(i, result[j + 1],
                                                                road_segments_result[i][result[j + 1]][2],
                                                                road_segments_result[i][result[j + 1]][0]))
        str = str + i + " "
        j = j + 1
